...
<template>
  <Transition name="fade">
    <div v-if="state.showSelectionOverlay" class="fixed inset-0 z-[1000] bg-bg-main flex flex-col">
      <header class="h-14 flex items-center px-4 justify-between border-b border-border-main flex-shrink-0">
      ...
      <div class="flex-1 overflow-y-auto p-4 select-text">
        <pre class="text-text-main text-sm leading-relaxed whitespace-pre-wrap break-words font-sans">{{ state.selectionText }}</pre>
      </div>
    </div>
  </Transition>
</template>
