...
<template>
  <aside 
    class="bg-bg-panel h-full flex flex-col transition-all duration-300 ease-in-out flex-shrink-0 z-[100] shadow-[1px_0_5px_rgba(0,0,0,0.05)] overflow-hidden"
    :class="[
      state.isSidebarOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0 md:opacity-100',
      state.isMobile ? 'fixed inset-y-0 left-0 w-72' : 'relative w-64',
      !state.isSidebarOpen && !state.isMobile ? 'md:w-0 md:translate-x-0 md:border-r-0 md:opacity-0' : 'border-r border-border-main'
    ]"
  >
